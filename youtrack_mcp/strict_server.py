...
class LegacyRouter:
    ...
    to canonical format with deprecation warnings.
    """

    def __init__(self, strict_server: StrictYouTrackMCPServer):
        """
        Initialize legacy router.

        Args:
            strict_server: The strict server to route to
        """
        self.strict_server = strict_server
        self.deprecation_warnings: Dict[str, bool] = {}

    def route_legacy_call(
        self,
        tool_name: str,
        args: tuple,
        kwargs: Dict[str, Any]
        ...
        if tool_name not in self.deprecation_warnings:
            logger.warning(
                f"Legacy tool call format detected for {tool_name}. "
                "Please use canonical JSON format: "
                '{"tool_name": "name", "arguments": {...}}'
            )
            self.deprecation_warnings[tool_name] = True

        # Convert legacy format to canonical format
        canonical_kwargs = self._convert_legacy_to_canonical(tool_name, args, kwargs)

        # Get the tool function from the strict server
        if tool_name in self.strict_server._tools:
            func = self.strict_server._tools[tool_name]
            return func(**canonical_kwargs)
        else:
            raise ValueError(f"Unknown tool: {tool_name}")

    def _convert_legacy_to_canonical(
        self,
        tool_name: str,
        args: tuple,
        kwargs: Dict[str, Any]
        ...
        """
        Convert legacy args/kwargs to canonical format.

        This is a simplified conversion - in practice, this would need
        to handle all the complex repair logic from the original wrappers.
        """
        ...
        param_mappings = {
            "issues.get": {"issue_id": args[0] if args else kwargs.get("issue_id")},
            "issues.create": {
                "project": args[0] if len(args) > 0 else kwargs.get("project"),
                "summary": args[1] if len(args) > 1 else kwargs.get("summary"),
        ...
        if tool_name in param_mappings:
            canonical = param_mappings[tool_name]
            # Remove None values
