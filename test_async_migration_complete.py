...
def create_async_mock_api():
    """Create mock API client with async methods."""
    mock_api = Mock()
    
    # Mock async methods that tools will call
    async def mock_get_issues(issue_id):
...
async def test_all_tools():
    ...
    with patch('youtrack_mcp.api.client.YouTrackClient') as MockClient:
        mock_client = Mock()
        mock_api, async_methods = create_async_mock_api()
        MockClient.return_value = mock_client
        
        # Configure mock client with async methods
        mock_client.get = async_methods["get"]
        mock_client.post = async_methods["post"]
        mock_client.put = async_methods["put"]
        mock_client.delete = async_methods["delete"]
        
        print("\n1. Testing CoreIssuesTools (3 methods)...")
        try:
            issues = CoreIssuesTools()
            issues.issues_api.get_issue = async_methods["get_issues"]
            issues.issues_api.create_issue = async_methods["create_issue"]
        ...
        try:
            ...
            projects.projects_api.get_projects = async_methods["get_projects"]
            projects.projects_api.get_project = async_methods["get_project"]
            projects.projects_api.create_project = async_methods["create_issue"]  # Reuse mock
        ...
        try:
            ...
            search.issues_api.search_issues = async_methods["search_issues"]
        ...
        try:
            ...
            users.users_api.search_users = async_methods["search_users"]
